...
(def continuation-types
  (->Registrar
    ...
    {:goto (make-lazy-entry "goto" frames/goto-frame-type)
     :prompt (make-lazy-entry "prompt" frames/prompt-frame-type)}))

(def iota-types
  (->Registrar
    (fn [] HexRegistries/IOTA_TYPE)
    (fn [] HexIotaTypes/REGISTRY)
    {:delimited-continuation (make-lazy-entry "continuation/delimited" iota.delimcc/iota-type)}))
